...
nms <- sub(".RDS", "", list.files("./data/datasets/complete_backup/categorical_as_factor/"))

sets_dim <- rbind(sets_dim, lapply(nms, function(i) {
  dat <- readRDS(paste0("./data/datasets/complete_backup/categorical_as_factor/", i, ".RDS"))
  data.frame(set_id = i,
             n_row = nrow(dat),
             n_col = ncol(dat),
